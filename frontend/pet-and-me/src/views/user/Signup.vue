<template>
    <div>
        ...
        <SignupInfo v-if="level == 1" @info-update="infoSave"> </SignupInfo>
        <SignupEmail v-if="level == 2" @email-update="emailSave"> </SignupEmail>
        <SignupPet v-if="level == 3" @pet-update="petSave"> </SignupPet>
        <SignupPreference v-if="level == 4" @preference-update="preferenceSave">
        </SignupPreference>
        <hr />
        ...
        <p>
            유저 애완동물 : (boolean + list) {{ credentials.petCheck }}
            {{ credentials.userPet }}
        </p>
